...
contract ZeroToBuidlGuidlNFTMintInfo is ERC721 {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                      STATE VARIABLES                       */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
    ...
    //@audit putting each of these into a MintInfo struct might make things tidier, could also then use smaller uint sizes to byte pack
    ...
    function mintNft() public payable {
        ...
        // s_tokenIdToTokenMinter[s_tokenCounter] = msg.sender;
        // s_tokenIdToBlockNumber[s_tokenCounter] = block.number;
        // s_tokenIdToDonation[s_tokenCounter] = msg.value;

        ++s_tokenCounter; // @audit ++s_tokenCounter for mini gas saving
    }
    ...
    function getThemeInfo(uint256 _id) external view returns(uint256) {
        if (s_tokenIdToMintInfo[_id].theme == Theme.DARK) {
            return 0;
        } else {
            return 1;
        }
