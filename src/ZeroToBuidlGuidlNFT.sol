...
contract ZeroToBuidlGuidlNFT is ERC721 {
    ...
    address private constant CURATOR_ADDRESS =
        0xfBD9Ca40386A8C632cf0529bbb16b4BEdB59a0A0;
    address private constant BUIDLGUIDL_ADDRESS =
        0x97843608a00e2bbc75ab0C1911387E002565DEDE;
    IReverseRecords private immutable ensReverseRecords;

    uint256 private s_tokenCounter; // @audit convention would be to name this totalSupply

    //@audit putting each of these into a MintInfo struct might make things tidier
    ...
    function mintNft() public payable {
        ...
        address tokenMinter = msg.sender; //@audit don't need to store msg.sender in memory

        // s_tokenCounter loaded from storage repeatedly, cache it in memory first and use that var
        _safeMint(tokenMinter, s_tokenCounter);

        // can pack info into struct to save on writing to storage (can be confident neither exceed uint128)
        // struct MintInfo {
        //     Theme theme;
        //     address minter;
        //     uint128 mintBlock;
        ...
        s_tokenIdToTheme[s_tokenCounter] = Theme.DARK; //@audit if this isn't set it'll be dark by default (DARK == 0)

        emit CreatedNFT(s_tokenCounter); // @audit solady function already emits a transfer event, not sure if second needed, track transfer events from address(0) to get mints
        s_tokenCounter = s_tokenCounter + 1; // @audit ++s_tokenCounter same, less gas
    }
    ...
    function withdrawFunds() public {
        ...
        (bool curatorSuccess, ) = payable(CURATOR_ADDRESS).call{
            value: curatorShare
        }("");
        if (!curatorSuccess) {
        ...
        (bool buidlguidlSuccess, ) = payable(BUIDLGUIDL_ADDRESS).call{
            value: buidlGuidlShare
        }("");
    ...
    function _buildTokenURI(uint256 id) internal view returns (string memory) {
        ...
        string memory backgroundColor = (currentTheme == Theme.DARK)
            ? "#2A3655"
            : "#F5F5F5";
        string memory textColor = (currentTheme == Theme.DARK)
            ? "#F5F5F5"
            : "#2A3655";
